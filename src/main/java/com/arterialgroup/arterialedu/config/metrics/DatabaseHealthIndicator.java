package com.arterialgroup.arterialedu.config.metrics;
...
public class DatabaseHealthIndicator extends AbstractHealthIndicator {
    ...
    private static String DISK_QUOTA_QUERY_MYSQL = "select sum((data_length+index_length)/1024/1024) AS MB from information_schema.tables where table_schema != 'information_schema';";
    ...
    protected void doHealthCheck(Health.Builder builder) throws Exception {
        ...
        if (StringUtils.hasText(query)) {
            try {
            	builder.withDetail("Disk Space (MB)", 
            			this.jdbcTemplate.queryForObject(DISK_QUOTA_QUERY_MYSQL, Object.class));
            } catch (Exception ex) {
                builder.down(ex);
            }
        }
